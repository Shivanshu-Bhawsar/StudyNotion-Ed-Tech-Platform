...
const InstructorDashboard = () => {
  ...
  const totalEarnings = instructorData?.reduce((acc, course) => acc + course?.totalRevenue, 0);
  const totalStudents = instructorData?.reduce((acc, course) => acc + course?.totalStudents, 0);

  return (
    <div className="mx-auto w-11/12 max-w-[1000px] py-7 sm:py-10">
      <div>
        ...
        <div className="my-4 flex space-x-6">
          {courses?.length === 0 ? (
            <p className="text-sm font-medium text-richblack-300">
              You have not created any courses yet, please first create course.
            </p>
          ) : (
